...
PROXYTYPE = os.getenv("PROXYTYPE", "HTTP")
...
INPUT_ERRORS = {"ERROR_PAGEURL", "ERROR_BAD_PARAMETERS", "ERROR_BAD_TOKEN_OR_PAGEURL"}
...
def validate_config() -> None:
    ...
    if not PROXY or PROXY == "login:password@ip:port":
        print("[!] ERROR: PROXY is not set.")
        print("    A proxy is required for Cloudflare Challenge.")
        print("    Set PROXY=login:password@ip:port in your .env file.")
        sys.exit(1)


def submit_task() -> str:
    """Submit the CAPTCHA task and return the task ID."""
    print("[*] Submitting Cloudflare Challenge task...")
    payload = {
        "key": API_KEY,
    ...
    if data.get("status") != 1:
        ...
        if error in AUTH_ERRORS:
            ...
            print("    Verify your page URL is correct.")
...
def poll_result(task_id: str) -> dict:
    ...
    print("[*] Waiting 20s before first poll...")
    time.sleep(20)
...
def main() -> None:
    ...
    print("[+] Solved!")
    print(f"[+] cf_clearance cookie: {result['cf_clearance']}")
    print(f"[+] User-Agent: {result['user_agent']}")
    print()
    print("Next step: set the cf_clearance cookie and User-Agent header")
    print("on all subsequent requests, using the same proxy IP.")
