...
const PROXYTYPE = process.env.PROXYTYPE || "HTTP";
...
const INPUT_ERRORS = new Set(["ERROR_PAGEURL", "ERROR_BAD_PARAMETERS", "ERROR_BAD_TOKEN_OR_PAGEURL"]);
...
function validateConfig() {
  ...
  if (!PROXY || PROXY === "login:password@ip:port") {
    console.log("[!] ERROR: PROXY is not set.");
    console.log("    A proxy is required for Cloudflare Challenge.");
    console.log("    Set PROXY=login:password@ip:port in your .env file.");
    process.exit(1);
  }
}

async function submitTask() {
  console.log("[*] Submitting Cloudflare Challenge task...");
  ...
  if (data.status !== 1) {
    ...
    if (AUTH_ERRORS.has(error)) {
      ...
      console.log("    Verify your page URL is correct.");
...
async function pollResult(taskId) {
  console.log("[*] Waiting 20s before first poll...");
  await sleep(20);
...
async function main() {
  ...
  console.log("[+] Solved!");
  console.log(`[+] cf_clearance cookie: ${result.cf_clearance}`);
  console.log(`[+] User-Agent: ${result.user_agent}`);
  console.log();
  console.log("Next step: set the cf_clearance cookie and User-Agent header");
  console.log("on all subsequent requests, using the same proxy IP.");
}
