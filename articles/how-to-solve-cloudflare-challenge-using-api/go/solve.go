...
var inputErrors = map[string]bool{"ERROR_PAGEURL": true, "ERROR_BAD_PARAMETERS": true, "ERROR_BAD_TOKEN_OR_PAGEURL": true}
...
func main() {
	...
	proxy := getEnv(env, "PROXY", "")
	proxytype := getEnv(env, "PROXYTYPE", "HTTP")
	...
	if proxy == "" || proxy == "login:password@ip:port" {
		fmt.Println("[!] ERROR: PROXY is not set.")
		fmt.Println("    A proxy is required for Cloudflare Challenge.")
		fmt.Println("    Set PROXY=login:password@ip:port in your .env file.")
		os.Exit(1)
	}
	...
	// Submit task
	fmt.Println("[*] Submitting Cloudflare Challenge task...")
	params := url.Values{
		"key":       {apiKey},
	...
	// Poll for result
	fmt.Println("[*] Waiting 20s before first poll...")
	...
	for elapsed < maxTimeout {
		...
		if result.Status == 1 {
			fmt.Println("[+] Solved!")
			fmt.Printf("[+] cf_clearance cookie: %s\n", result.Request)
			fmt.Printf("[+] User-Agent: %s\n", result.UserAgent)
			fmt.Println()
			fmt.Println("Next step: set the cf_clearance cookie and User-Agent header")
			fmt.Println("on all subsequent requests, using the same proxy IP.")
			return
		}
...
func handleError(errCode string) {
	if authErrors[errCode] {
		...
		fmt.Println("    Verify your page URL is correct.")
