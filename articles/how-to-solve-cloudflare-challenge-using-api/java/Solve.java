...
public class Solve {
    ...
    static final Set<String> INPUT_ERRORS = Set.of("ERROR_PAGEURL", "ERROR_BAD_PARAMETERS", "ERROR_BAD_TOKEN_OR_PAGEURL");
    ...
    public static void main(String[] args) throws Exception {
        ...
        String proxy = getEnv("PROXY", "");
        String proxytype = getEnv("PROXYTYPE", "HTTP");
        ...
        if (proxy.isEmpty() || proxy.equals("login:password@ip:port")) {
            System.out.println("[!] ERROR: PROXY is not set.");
            System.out.println("    A proxy is required for Cloudflare Challenge.");
            System.out.println("    Set PROXY=login:password@ip:port in your .env file.");
            System.exit(1);
        }

        // Submit task
        System.out.println("[*] Submitting Cloudflare Challenge task...");
        String submitParams = String.format(
            "key=%s&method=cloudflare_challenge&pageurl=%s&proxy=%s&proxytype=%s&json=1",
            enc(apiKey), enc(pageurl), enc(proxy), enc(proxytype)
        );
        ...
        // Poll for result
        System.out.println("[*] Waiting 20s before first poll...");
        Thread.sleep(20000);
        ...
        while (elapsed < maxTimeout) {
            ...
            if (status == 1) {
                String userAgent = jsonStr(resultResp, "user_agent");
                System.out.println("[+] Solved!");
                System.out.println("[+] cf_clearance cookie: " + request);
                System.out.println("[+] User-Agent: " + userAgent);
                System.out.println();
                System.out.println("Next step: set the cf_clearance cookie and User-Agent header");
                System.out.println("on all subsequent requests, using the same proxy IP.");
                return;
            }
    ...
    static void handleError(String error) {
        if (AUTH_ERRORS.contains(error)) {
            ...
            System.out.println("    Verify your page URL is correct.");
