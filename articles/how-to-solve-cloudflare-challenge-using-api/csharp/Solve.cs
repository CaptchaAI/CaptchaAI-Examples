...
var inputErrors = new HashSet<string> { "ERROR_PAGEURL", "ERROR_BAD_PARAMETERS", "ERROR_BAD_TOKEN_OR_PAGEURL" };
...
string proxy = GetEnv(env, "PROXY", "");
string proxytype = GetEnv(env, "PROXYTYPE", "HTTP");
...
if (string.IsNullOrEmpty(proxy) || proxy == "login:password@ip:port")
{
    Console.WriteLine("[!] ERROR: PROXY is not set.");
    Console.WriteLine("    A proxy is required for Cloudflare Challenge.");
    Console.WriteLine("    Set PROXY=login:password@ip:port in your .env file.");
    Environment.Exit(1);
}
...
// Submit task
Console.WriteLine("[*] Submitting Cloudflare Challenge task...");
var submitQuery = $"key={Uri.EscapeDataString(apiKey)}&method=cloudflare_challenge&pageurl={Uri.EscapeDataString(pageurl)}&proxy={Uri.EscapeDataString(proxy)}&proxytype={Uri.EscapeDataString(proxytype)}&json=1";
...
// Poll for result
Console.WriteLine("[*] Waiting 20s before first poll...");
...
while (elapsed < maxTimeout)
{
    ...
    {
        string userAgent = "";
        if (resultDoc.RootElement.TryGetProperty("user_agent", out var uaProp))
            userAgent = uaProp.GetString() ?? "";
        Console.WriteLine("[+] Solved!");
        Console.WriteLine($"[+] cf_clearance cookie: {request}");
        Console.WriteLine($"[+] User-Agent: {userAgent}");
        Console.WriteLine();
        Console.WriteLine("Next step: set the cf_clearance cookie and User-Agent header");
        Console.WriteLine("on all subsequent requests, using the same proxy IP.");
        return;
    }
...
static void HandleError(string error, HashSet<string> auth, HashSet<string> balance, HashSet<string> input, HashSet<string> proxy)
{
    ...
    else if (input.Contains(error))
    {
        ...
        Console.WriteLine("    Verify your page URL is correct.");
    }
