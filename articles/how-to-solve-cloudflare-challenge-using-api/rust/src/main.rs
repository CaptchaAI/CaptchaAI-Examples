...
fn handle_error(error: &str) {
    ...
    let input = ["ERROR_PAGEURL", "ERROR_BAD_PARAMETERS", "ERROR_BAD_TOKEN_OR_PAGEURL"];
...
fn main() {
    ...
    let proxy = get_env(&env, "PROXY", "");
    let proxytype = get_env(&env, "PROXYTYPE", "HTTP");
    ...
    if proxy.is_empty() || proxy == "login:password@ip:port" {
    ...
    let submit_url = format!(
        "{}?key={}&method=cloudflare_challenge&pageurl={}&proxy={}&proxytype={}&json=1",
        SUBMIT_URL,
        urlencoding::encode(&api_key),
        urlencoding::encode(&pageurl),
        urlencoding::encode(&proxy),
        urlencoding::encode(&proxytype)
    );
    ...
    while elapsed < max_timeout {
        ...
        if status == 1 {
            let user_agent = json_str(&result_resp, "user_agent");
            println!("[+] Solved!");
