...
val INPUT_ERRORS = setOf("ERROR_PAGEURL", "ERROR_BAD_PARAMETERS", "ERROR_BAD_TOKEN_OR_PAGEURL")
...
fun handleError(error: String) {
    when {
        ...
        error in INPUT_ERRORS -> {
            ...
            println("    Verify your page URL is correct.")
        }
...
fun main() {
    ...
    val proxy = getEnv(env, "PROXY")
    val proxytype = getEnv(env, "PROXYTYPE", "HTTP")
    ...
    if (proxy.isEmpty() || proxy == "login:password@ip:port") {
        println("[!] ERROR: PROXY is not set.")
        println("    A proxy is required for Cloudflare Challenge.")
        println("    Set PROXY=login:password@ip:port in your .env file.")
        System.exit(1)
    }

    // Submit task
    println("[*] Submitting Cloudflare Challenge task...")
    val submitParams = "key=${enc(apiKey)}&method=cloudflare_challenge&pageurl=${enc(pageurl)}&proxy=${enc(proxy)}&proxytype=${enc(proxytype)}&json=1"
    ...
    // Poll for result
    println("[*] Waiting 20s before first poll...")
    Thread.sleep(20000)
    ...
    while (elapsed < maxTimeout) {
        ...
        if (status == 1) {
            val userAgent = jsonStr(resultResp, "user_agent")
            println("[+] Solved!")
            println("[+] cf_clearance cookie: $request")
            println("[+] User-Agent: $userAgent")
            println()
            println("Next step: set the cf_clearance cookie and User-Agent header")
            println("on all subsequent requests, using the same proxy IP.")
            return
        }
