...
class Solve
{
    ...
    private async Task<string> SubmitTaskAsync()
    {
        ...
        var query = $"key={Uri.EscapeDataString(_apiKey)}" +
                    ...
                    $"&proxy={Uri.EscapeDataString(_proxy)}" +
                    $"&proxytype={Uri.EscapeDataString(_proxytype)}" +
                    $"&json=1";
    ...
    private async Task<string> PollResultAsync(string taskId)
    {
        ...
        var elapsed = 20; var attempt = 0; var backoff = _pollInterval;
