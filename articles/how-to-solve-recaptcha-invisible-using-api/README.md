# reCAPTCHA Invisible — CaptchaAI Example Pack
...
**[How to Solve reCAPTCHA Invisible Using API](https://blog.captchaai.com/how-to-solve-recaptcha-invisible-using-api)**
...
1. Clone the repository:
   ...
   cd CaptchaAI-Examples/articles/how-to-solve-recaptcha-invisible-using-api
   ```
...
3. Edit `.env` with your API key and the target page's sitekey and URL.
...
| `CAPTCHA_SITEKEY` | reCAPTCHA Invisible sitekey from the target page | Yes |
...
1. **Submit** — Sends the CAPTCHA parameters to `https://ocr.captchaai.com/in.php` using method `userrecaptcha` with `invisible=1`
...
- Make sure `.env` file is in the `articles/how-to-solve-recaptcha-invisible-using-api/` directory (not inside a language folder)
...
- Some invisible reCAPTCHAs use a `data-callback` function — call it with the token instead
...
- [reCAPTCHA v3 Enterprise Example](../how-to-solve-recaptcha-v3-enterprise-using-api/) — Enterprise v3 solving
