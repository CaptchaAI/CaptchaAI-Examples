...
if (string.IsNullOrEmpty(sitekey) || sitekey == "YOUR_SITE_KEY")
{
    ...
    Console.WriteLine("    Add the reCAPTCHA Invisible sitekey from your target page to .env");
    Environment.Exit(1);
}
...
// Submit task
Console.WriteLine("[*] Submitting reCAPTCHA Invisible task...");
var submitQuery = $"key={Uri.EscapeDataString(apiKey)}&method=userrecaptcha&googlekey={Uri.EscapeDataString(sitekey)}&pageurl={Uri.EscapeDataString(pageurl)}&invisible=1&json=1";
...
while (elapsed < maxTimeout)
{
    ...
    if (status == 1)
    {
        ...
        Console.WriteLine();
        Console.WriteLine("Next step: inject this token via the page's data-callback function");
        Console.WriteLine("or set the g-recaptcha-response hidden field and submit the form.");
        return;
    }
