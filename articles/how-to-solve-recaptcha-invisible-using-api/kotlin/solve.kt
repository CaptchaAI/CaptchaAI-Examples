...
fun main() {
    ...
    if (sitekey.isEmpty() || sitekey == "YOUR_SITE_KEY") {
        ...
        println("    Add the reCAPTCHA Invisible sitekey from your target page to .env")
        System.exit(1)
    }
    ...
    // Submit task
    println("[*] Submitting reCAPTCHA Invisible task...")
    val submitParams = "key=${enc(apiKey)}&method=userrecaptcha&googlekey=${enc(sitekey)}&pageurl=${enc(pageurl)}&invisible=1&json=1"
    ...
    while (elapsed < maxTimeout) {
        ...
        if (status == 1) {
            ...
            println()
            println("Next step: inject this token via the page's data-callback function")
            println("or set the g-recaptcha-response hidden field and submit the form.")
            return
        }
