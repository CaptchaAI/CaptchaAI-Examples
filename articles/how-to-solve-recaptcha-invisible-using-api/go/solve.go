...
func main() {
	...
	if sitekey == "" || sitekey == "YOUR_SITE_KEY" {
		...
		fmt.Println("    Add the reCAPTCHA Invisible sitekey from your target page to .env")
		os.Exit(1)
	}
	...
	// Submit task
	fmt.Println("[*] Submitting reCAPTCHA Invisible task...")
	params := url.Values{
		"key":       {apiKey},
	...
	for elapsed < maxTimeout {
		...
		if result.Status == 1 {
			...
			fmt.Println()
			fmt.Println("Next step: inject this token via the page's data-callback function")
			fmt.Println("or set the g-recaptcha-response hidden field and submit the form.")
			return
		}
