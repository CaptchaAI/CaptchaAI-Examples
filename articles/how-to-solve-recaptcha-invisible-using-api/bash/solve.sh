...
if [[ -z "$SITEKEY" || "$SITEKEY" == "YOUR_SITE_KEY" ]]; then
  ...
  echo "    Add the reCAPTCHA Invisible sitekey from your target page to .env"
  exit 1
fi
...
# Submit task
echo "[*] Submitting reCAPTCHA Invisible task..."
...
SUBMIT_QUERY="${SUBMIT_URL}?key=${ENCODED_KEY}&method=userrecaptcha&googlekey=${ENCODED_SITEKEY}&pageurl=${ENCODED_PAGEURL}&invisible=1&json=1"
...
while [[ "$ELAPSED" -lt "$MAX_TIMEOUT" ]]; do
  ...
  if [[ "$STATUS" -eq 1 ]]; then
    ...
    echo
    echo "Next step: inject this token via the page's data-callback function"
    echo "or set the g-recaptcha-response hidden field and submit the form."
    exit 0
  fi
