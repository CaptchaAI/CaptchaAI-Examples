...
fun main() {
    ...
    if (instructions.isEmpty()) {
        ...
        println("    Provide text instructions (e.g. 'Select all images with a car').")
        System.exit(1)
    }
    ...
    println("[*] Submitting BLS CAPTCHA task...")
    val postData = "key=${enc(apiKey)}&method=bls&body=${enc(imageB64)}&textinstructions=${enc(instructions)}&json=1"
    ...
    while (elapsed < maxTimeout) {
        ...
        if (status == 1) {
            ...
            println()
            println("Next step: use these selected indices to click the")
            println("corresponding images on the target page and submit.")
            return
        }
