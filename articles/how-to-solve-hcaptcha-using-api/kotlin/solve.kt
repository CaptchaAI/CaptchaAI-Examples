...
fun main() {
    ...
    if (sitekey.isEmpty() || sitekey == "YOUR_SITE_KEY") {
        ...
        println("    Add the hCaptcha sitekey from your target page to .env")
        System.exit(1)
    }
    ...
    // Submit task
    println("[*] Submitting hCaptcha task...")
    val submitParams = "key=${enc(apiKey)}&method=hcaptcha&sitekey=${enc(sitekey)}&pageurl=${enc(pageurl)}&json=1"
    ...
    while (elapsed < maxTimeout) {
        ...
        if (status == 1) {
            ...
            println("h-captcha-response hidden field and submit the form.")
            return
        }
