...
if [[ -z "$SITEKEY" || "$SITEKEY" == "YOUR_SITE_KEY" ]]; then
  ...
  echo "    Add the hCaptcha sitekey from your target page to .env"
  exit 1
fi
...
SUBMIT_RESP=$(curl -sS --max-time 30 \
  "${SUBMIT_URL}?key=${ENCODED_KEY}&method=hcaptcha&sitekey=${ENCODED_SITEKEY}&pageurl=${ENCODED_PAGEURL}&json=1")
...
while [[ "$ELAPSED" -lt "$MAX_TIMEOUT" ]]; do
  ...
  if [[ "$STATUS" -eq 1 ]]; then
    ...
    echo "h-captcha-response hidden field and submit the form."
    exit 0
  fi
