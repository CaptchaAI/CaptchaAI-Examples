...
if (string.IsNullOrEmpty(sitekey) || sitekey == "YOUR_SITE_KEY")
{
    ...
    Console.WriteLine("    Add the hCaptcha sitekey from your target page to .env");
    Environment.Exit(1);
}
...
// Submit task
Console.WriteLine("[*] Submitting hCaptcha task...");
var submitQuery = $"key={Uri.EscapeDataString(apiKey)}&method=hcaptcha&sitekey={Uri.EscapeDataString(sitekey)}&pageurl={Uri.EscapeDataString(pageurl)}&json=1";
...
while (elapsed < maxTimeout)
{
    ...
    if (status == 1)
    {
        ...
        Console.WriteLine("h-captcha-response hidden field and submit the form.");
        return;
    }
