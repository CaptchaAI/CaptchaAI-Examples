...
api_server = get_env(env, "CAPTCHA_API_SERVER")
...
if gt.empty? || gt == "YOUR_GT_VALUE"
  puts "[!] ERROR: CAPTCHA_GT is not set."
  puts "    Find the gt value in the page source or GeeTest initialization call."
  exit 1
end
if challenge.empty? || challenge == "YOUR_CHALLENGE_VALUE"
  puts "[!] ERROR: CAPTCHA_CHALLENGE is not set."
  puts "    The challenge token must be fresh — fetch a new one for each solve attempt."
  exit 1
end
...
def handle_error(error)
  if AUTH_ERRORS.include?(error)
    ...
    puts "    Verify your gt, challenge, and page URL are correct."
...
submit_params[:api_server] = api_server unless api_server.empty?
params = URI.encode_www_form(submit_params)
...
while elapsed < max_timeout
  ...
  if result_data["status"] == 1
    ...
    solution = JSON.parse(solution) if solution.is_a?(String)
    puts "[+] Solved!"
    puts "[+] geetest_challenge: #{solution['geetest_challenge']}"
    puts "[+] geetest_validate: #{solution['geetest_validate']}"
    puts "[+] geetest_seccode: #{solution['geetest_seccode']}"
    puts
    puts "Next step: submit these three values to the site's"
    puts "GeeTest verification endpoint."
    exit 0
  end
