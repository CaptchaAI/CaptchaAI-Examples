...
fn extract_request_json(json: &str) -> String {
    let pattern = "\"request\":";
    if let Some(idx) = json.find(pattern) {
        let start = idx + pattern.len();
        let rest = json[start..].trim_start();
        if rest.starts_with('"') {
            // JSON string — find closing quote, handle escapes
            let inner = &rest[1..];
            let mut end = 0;
            ...
            let s: String = inner.chars().take(end).collect();
...
fn main() {
    ...
    let api_server = get_env(&env, "CAPTCHA_API_SERVER", "");
    ...
    if pageurl.is_empty() || pageurl == "https://example.com" {
    ...
    let mut submit_url = format!(
        "{}?key={}&method=geetest&gt={}&challenge={}&pageurl={}&json=1",
        SUBMIT_URL,
        urlencoding::encode(&api_key),
        urlencoding::encode(&gt),
        urlencoding::encode(&challenge),
        urlencoding::encode(&pageurl)
    );
    if !api_server.is_empty() {
        submit_url.push_str(&format!("&api_server={}", urlencoding::encode(&api_server)));
    }
    ...
    while elapsed < max_timeout {
        ...
        if status == 1 {
            ...
            let solution_json = extract_request_json(&result_resp);
            let geetest_challenge = json_str(&solution_json, "geetest_challenge");
            let geetest_validate = json_str(&solution_json, "geetest_validate");
            let geetest_seccode = json_str(&solution_json, "geetest_seccode");
            println!("[+] Solved!");
            println!("[+] geetest_challenge: {}", geetest_challenge);
            println!("[+] geetest_validate: {}", geetest_validate);
            println!("[+] geetest_seccode: {}", geetest_seccode);
            println!();
