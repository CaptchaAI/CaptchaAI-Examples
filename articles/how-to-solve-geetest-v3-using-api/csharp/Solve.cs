...
string challenge = GetEnv(env, "CAPTCHA_CHALLENGE", "");
string pageurl = GetEnv(env, "CAPTCHA_PAGEURL", "");
string apiServer = GetEnv(env, "CAPTCHA_API_SERVER", "");
...
if (string.IsNullOrEmpty(gt) || gt == "YOUR_GT_VALUE")
{
    Console.WriteLine("[!] ERROR: CAPTCHA_GT is not set.");
    Console.WriteLine("    Find the gt value in the page source or GeeTest initialization call.");
    Environment.Exit(1);
}
if (string.IsNullOrEmpty(challenge) || challenge == "YOUR_CHALLENGE_VALUE")
{
    Console.WriteLine("[!] ERROR: CAPTCHA_CHALLENGE is not set.");
    Console.WriteLine("    The challenge token must be fresh — fetch a new one for each solve attempt.");
    Environment.Exit(1);
}
...
var submitQuery = $"key={Uri.EscapeDataString(apiKey)}&method=geetest&gt={Uri.EscapeDataString(gt)}&challenge={Uri.EscapeDataString(challenge)}&pageurl={Uri.EscapeDataString(pageurl)}&json=1";
if (!string.IsNullOrEmpty(apiServer))
{
    submitQuery += $"&api_server={Uri.EscapeDataString(apiServer)}";
}
...
while (elapsed < maxTimeout)
{
    ...
    if (status == 1)
    {
        ...
        string geetestChallenge = solutionDoc.RootElement.GetProperty("geetest_challenge").GetString() ?? "";
        string geetestValidate = solutionDoc.RootElement.GetProperty("geetest_validate").GetString() ?? "";
        string geetestSeccode = solutionDoc.RootElement.GetProperty("geetest_seccode").GetString() ?? "";
        solutionDoc.Dispose();

        Console.WriteLine("[+] Solved!");
        Console.WriteLine($"[+] geetest_challenge: {geetestChallenge}");
        Console.WriteLine($"[+] geetest_validate: {geetestValidate}");
        Console.WriteLine($"[+] geetest_seccode: {geetestSeccode}");
        Console.WriteLine();
        Console.WriteLine("Next step: submit these three values to the site's");
        Console.WriteLine("GeeTest verification endpoint.");
        return;
    }
...
static void HandleError(string error, HashSet<string> auth, HashSet<string> balance, HashSet<string> input, HashSet<string> proxy)
{
    ...
    else if (input.Contains(error))
    {
        ...
        Console.WriteLine("    Verify your gt, challenge, and page URL are correct.");
    }
