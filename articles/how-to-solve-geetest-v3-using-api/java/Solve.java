...
public class Solve {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String challenge = getEnv("CAPTCHA_CHALLENGE", "");
        String pageurl = getEnv("CAPTCHA_PAGEURL", "");
        String apiServer = getEnv("CAPTCHA_API_SERVER", "");
        ...
        if (gt.isEmpty() || gt.equals("YOUR_GT_VALUE")) {
            System.out.println("[!] ERROR: CAPTCHA_GT is not set.");
            System.out.println("    Find the gt value in the page source or GeeTest initialization call.");
            System.exit(1);
        }
        if (challenge.isEmpty() || challenge.equals("YOUR_CHALLENGE_VALUE")) {
            System.out.println("[!] ERROR: CAPTCHA_CHALLENGE is not set.");
            System.out.println("    The challenge token must be fresh — fetch a new one for each solve attempt.");
            System.exit(1);
        }
        ...
        // Submit task
        System.out.println("[*] Submitting GeeTest v3 task...");
        String submitParams = String.format(
            "key=%s&method=geetest&gt=%s&challenge=%s&pageurl=%s&json=1",
            enc(apiKey), enc(gt), enc(challenge), enc(pageurl)
        );
        ...
        while (elapsed < maxTimeout) {
            ...
            if (status == 1) {
                // The request field contains a JSON object with the 3 GeeTest values
                // Extract the JSON object from the request field
                String requestValue = extractRequestValue(resultResp);
                String geetestChallenge = jsonStr(requestValue, "geetest_challenge");
                String geetestValidate = jsonStr(requestValue, "geetest_validate");
                String geetestSeccode = jsonStr(requestValue, "geetest_seccode");
                System.out.println("[+] Solved!");
                System.out.println("[+] geetest_challenge: " + geetestChallenge);
                System.out.println("[+] geetest_validate: " + geetestValidate);
                System.out.println("[+] geetest_seccode: " + geetestSeccode);
                System.out.println();
                System.out.println("Next step: submit these three values to the site's");
                System.out.println("GeeTest verification endpoint.");
                return;
            }
    ...
    static void handleError(String error) {
        if (AUTH_ERRORS.contains(error)) {
            ...
            System.out.println("    Verify your gt, challenge, and page URL are correct.");
