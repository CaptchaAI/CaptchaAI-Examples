...
fun extractRequestJson(json: String): String {
    val pattern = "\"request\":"
    val idx = json.indexOf(pattern)
    if (idx < 0) return ""
    val rest = json.substring(idx + pattern.length).trimStart()
    if (rest.startsWith("\"")) {
        // JSON string — find closing quote with escape handling
        ...
        return inner.substring(0, end).replace("\\\"", "\"").replace("\\\\", "\\")
    } else if (rest.startsWith("{")) {
        // JSON object — find matching brace
        var depth = 0
        for (i in rest.indices) {
            if (rest[i] == '{') depth++
            else if (rest[i] == '}') { depth--; if (depth == 0) return rest.substring(0, i + 1) }
        }
    }
    return ""
}
...
fun handleError(error: String) {
    when {
        ...
        error in INPUT_ERRORS -> {
            ...
            println("    Verify your gt, challenge, and page URL are correct.")
        }
...
fun main() {
    ...
    val apiServer = getEnv(env, "CAPTCHA_API_SERVER")
    ...
    if (gt.isEmpty() || gt == "YOUR_GT_VALUE") {
        println("[!] ERROR: CAPTCHA_GT is not set.")
        println("    Find the gt value in the page source or GeeTest initialization call.")
        System.exit(1)
    }
    if (challenge.isEmpty() || challenge == "YOUR_CHALLENGE_VALUE") {
        println("[!] ERROR: CAPTCHA_CHALLENGE is not set.")
        println("    The challenge token must be fresh — fetch a new one for each solve attempt.")
        System.exit(1)
    }
    if (pageurl.isEmpty() || pageurl == "https://example.com") {
    ...
    var submitParams = "key=${enc(apiKey)}&method=geetest&gt=${enc(gt)}&challenge=${enc(challenge)}&pageurl=${enc(pageurl)}&json=1"
    if (apiServer.isNotEmpty()) {
        submitParams += "&api_server=${enc(apiServer)}"
    }
    ...
    while (elapsed < maxTimeout) {
        ...
        if (status == 1) {
            // The request field contains a JSON string or object with the 3 GeeTest values
            val solutionJson = extractRequestJson(resultResp)
            val geetestChallenge = jsonStr(solutionJson, "geetest_challenge")
            val geetestValidate = jsonStr(solutionJson, "geetest_validate")
            val geetestSeccode = jsonStr(solutionJson, "geetest_seccode")
            println("[+] Solved!")
            println("[+] geetest_challenge: $geetestChallenge")
            println("[+] geetest_validate: $geetestValidate")
            println("[+] geetest_seccode: $geetestSeccode")
            println()
            println("Next step: submit these three values to the site's")
            println("GeeTest verification endpoint.")
            return
        }
