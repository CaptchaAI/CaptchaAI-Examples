/**
 * Solve reCAPTCHA v2 (Callback) using the CaptchaAI API.
 *
 * Usage:
 *   npm install
...
function validateConfig() {
  ...
  if (!CAPTCHA_SITEKEY || CAPTCHA_SITEKEY === "YOUR_SITE_KEY") {
    console.log("[!] ERROR: CAPTCHA_SITEKEY is not set.");
    console.log("    Add the reCAPTCHA v2 sitekey from your target page to .env");
    process.exit(1);
  }
  if (!CAPTCHA_PAGEURL || CAPTCHA_PAGEURL === "https://example.com/login") {
...
async function submitTask() {
  console.log("[*] Submitting reCAPTCHA v2 (Callback) task...");
...
async function main() {
  ...
  console.log();
  console.log("Next step: call the page's reCAPTCHA callback function with this token.");
  console.log("Find the callback via data-callback attribute, grecaptcha.render(),");
  console.log("or ___grecaptcha_cfg.clients[0], then invoke: callbackName(token)");
}
