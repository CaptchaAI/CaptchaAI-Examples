...
public class Solve {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (sitekey.isEmpty() || sitekey.equals("YOUR_SITE_KEY")) {
            ...
            System.out.println("    Add the reCAPTCHA v2 sitekey from your target page to .env");
            System.exit(1);
        }
        ...
        // Submit task
        System.out.println("[*] Submitting reCAPTCHA v2 (Callback) task...");
        String submitParams = String.format(
            "key=%s&method=userrecaptcha&googlekey=%s&pageurl=%s&json=1",
            enc(apiKey), enc(sitekey), enc(pageurl)
        );
        ...
        while (elapsed < maxTimeout) {
            ...
            if (status == 1) {
                ...
                System.out.println();
                System.out.println("Next step: call the page's reCAPTCHA callback function with this token.");
                System.out.println("Find the callback via data-callback attribute, grecaptcha.render(),");
                System.out.println("or ___grecaptcha_cfg.clients[0], then invoke: callbackName(token)");
                return;
            }
