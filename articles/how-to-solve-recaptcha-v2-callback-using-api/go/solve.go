...
func main() {
	...
	if sitekey == "" || sitekey == "YOUR_SITE_KEY" {
		...
		fmt.Println("    Add the reCAPTCHA v2 sitekey from your target page to .env")
		os.Exit(1)
	}
	...
	// Submit task
	fmt.Println("[*] Submitting reCAPTCHA v2 (Callback) task...")
	params := url.Values{
		"key":       {apiKey},
	...
	for elapsed < maxTimeout {
		...
		if result.Status == 1 {
			...
			fmt.Println()
			fmt.Println("Next step: call the page's reCAPTCHA callback function with this token.")
			fmt.Println("Find the callback via data-callback attribute, grecaptcha.render(),")
			fmt.Println("or ___grecaptcha_cfg.clients[0], then invoke: callbackName(token)")
			return
		}
