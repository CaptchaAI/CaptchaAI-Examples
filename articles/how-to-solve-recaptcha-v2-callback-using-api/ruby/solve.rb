...
if sitekey.empty? || sitekey == "YOUR_SITE_KEY"
  ...
  puts "    Add the reCAPTCHA v2 sitekey from your target page to .env"
  exit 1
end
...
# Submit task
puts "[*] Submitting reCAPTCHA v2 (Callback) task..."
...
while elapsed < max_timeout
  ...
  if result_data["status"] == 1
    ...
    puts
    puts "Next step: call the page's reCAPTCHA callback function with this token."
    puts "Find the callback via data-callback attribute, grecaptcha.render(),"
    puts "or ___grecaptcha_cfg.clients[0], then invoke: callbackName(token)"
    exit 0
  end
