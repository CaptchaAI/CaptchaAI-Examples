...
fun main() {
    ...
    if (sitekey.isEmpty() || sitekey == "YOUR_SITE_KEY") {
        ...
        println("    Add the reCAPTCHA v2 sitekey from your target page to .env")
        System.exit(1)
    }
    ...
    // Submit task
    println("[*] Submitting reCAPTCHA v2 (Callback) task...")
    val submitParams = "key=${enc(apiKey)}&method=userrecaptcha&googlekey=${enc(sitekey)}&pageurl=${enc(pageurl)}&json=1"
    ...
    while (elapsed < maxTimeout) {
        ...
        if (status == 1) {
            ...
            println()
            println("Next step: call the page's reCAPTCHA callback function with this token.")
            println("Find the callback via data-callback attribute, grecaptcha.render(),")
            println("or ___grecaptcha_cfg.clients[0], then invoke: callbackName(token)")
            return
        }
