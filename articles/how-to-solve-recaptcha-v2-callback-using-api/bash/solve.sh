...
if [[ -z "$SITEKEY" || "$SITEKEY" == "YOUR_SITE_KEY" ]]; then
  ...
  echo "    Add the reCAPTCHA v2 sitekey from your target page to .env"
  exit 1
fi
...
# Submit task
echo "[*] Submitting reCAPTCHA v2 (Callback) task..."
...
SUBMIT_RESP=$(curl -sS --max-time 30 \
  "${SUBMIT_URL}?key=${ENCODED_KEY}&method=userrecaptcha&googlekey=${ENCODED_SITEKEY}&pageurl=${ENCODED_PAGEURL}&json=1")
...
while [[ "$ELAPSED" -lt "$MAX_TIMEOUT" ]]; do
  ...
  if [[ "$STATUS" -eq 1 ]]; then
    ...
    echo
    echo "Next step: call the page's reCAPTCHA callback function with this token."
    echo "Find the callback via data-callback attribute, grecaptcha.render(),"
    echo "or ___grecaptcha_cfg.clients[0], then invoke: callbackName(token)"
    exit 0
  fi
