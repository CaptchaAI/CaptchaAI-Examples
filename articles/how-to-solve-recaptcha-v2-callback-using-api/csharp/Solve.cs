...
// Submit task
Console.WriteLine("[*] Submitting reCAPTCHA v2 (Callback) task...");
...
while (elapsed < maxTimeout)
{
    ...
    if (status == 1)
    {
        ...
        Console.WriteLine();
        Console.WriteLine("Next step: call the page's reCAPTCHA callback function with this token.");
        Console.WriteLine("Find the callback via data-callback attribute, grecaptcha.render(),");
        Console.WriteLine("or ___grecaptcha_cfg.clients[0], then invoke: callbackName(token)");
        return;
    }
