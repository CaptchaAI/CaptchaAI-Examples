...
def validate_config() -> None:
    ...
    if not CAPTCHA_SITEKEY or CAPTCHA_SITEKEY == "YOUR_SITE_KEY":
        ...
        print("    Add the reCAPTCHA v2 sitekey from your target page to .env")
        sys.exit(1)
...
def submit_task() -> str:
    ...
    print("[*] Submitting reCAPTCHA v2 (Callback) task...")
    payload = {
        "key": API_KEY,
...
def main() -> None:
    ...
    print()
    print("Next step: call the page's reCAPTCHA callback function with this token.")
    print("Find the callback via data-callback attribute, grecaptcha.render(),")
    print("or ___grecaptcha_cfg.clients[0], then invoke: callbackName(token)")
