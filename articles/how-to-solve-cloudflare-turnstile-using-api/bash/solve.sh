...
if [[ -z "$SITEKEY" || "$SITEKEY" == "YOUR_SITE_KEY" ]]; then
  ...
  echo "    Add the Cloudflare Turnstile sitekey from your target page to .env"
  exit 1
fi
...
SUBMIT_RESP=$(curl -sS --max-time 30 \
  "${SUBMIT_URL}?key=${ENCODED_KEY}&method=turnstile&sitekey=${ENCODED_SITEKEY}&pageurl=${ENCODED_PAGEURL}&json=1")
