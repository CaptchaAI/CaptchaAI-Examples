...
if (string.IsNullOrEmpty(sitekey) || sitekey == "YOUR_SITE_KEY")
{
    ...
    Console.WriteLine("    Add the Cloudflare Turnstile sitekey from your target page to .env");
    Environment.Exit(1);
}
...
var submitQuery = $"key={Uri.EscapeDataString(apiKey)}&method=turnstile&sitekey={Uri.EscapeDataString(sitekey)}&pageurl={Uri.EscapeDataString(pageurl)}&json=1";
