...
class Solve
{
    ...
    {
        Console.WriteLine("[*] Submitting Image/OCR CAPTCHA task...");
        var query = $"key={Uri.EscapeDataString(_apiKey)}" +
                    $"&method=base64" +
                    $"&body={Uri.EscapeDataString(_body)}" +
                    $"&json=1";
    ...
    private async Task<string> PollResultAsync(string taskId)
    {
        ...
        var elapsed = 10; var attempt = 0; var backoff = _pollInterval;
