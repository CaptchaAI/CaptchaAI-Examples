...
fun handleError(error: String) {
    when {
        ...
        error in INPUT_ERRORS -> {
            ...
            println("    Verify your image and instructions are correct.")
        }
...
fun main() {
    ...
    val imageFile = getEnv(env, "CAPTCHA_IMAGE_FILE")
    val instructions = getEnv(env, "CAPTCHA_INSTRUCTIONS")
    ...
    if (imageFile.isEmpty()) {
        ...
        println("    Set the path to the grid CAPTCHA image file in .env.")
        System.exit(1)
    }
    ...
    if (instructions.isEmpty()) {
        println("[!] ERROR: CAPTCHA_INSTRUCTIONS is not set.")
        println("    Provide text instructions (e.g. 'Select all squares with traffic lights').")
        System.exit(1)
    }

    // Load image and base64-encode
    println("[*] Loading grid image from $imageFile...")
    ...
    val postData = "key=${enc(apiKey)}&method=post&body=${enc(imageB64)}&recaptcha=1&textinstructions=${enc(instructions)}&json=1"
    ...
    while (elapsed < maxTimeout) {
        ...
        if (status == 1) {
            val answer = jsonStr(resultResp, "request")
            println("[+] Solved! Selected cells: $answer")
            println()
            println("Next step: click the cells at the returned indices on")
            println("the target page's grid and submit.")
            return
        }
