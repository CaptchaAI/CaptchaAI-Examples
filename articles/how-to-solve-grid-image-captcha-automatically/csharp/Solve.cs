...
if (string.IsNullOrEmpty(imageFile))
{
    ...
    Console.WriteLine("    Set the path to the grid CAPTCHA image file in .env.");
    Environment.Exit(1);
}
...
if (string.IsNullOrEmpty(instructions))
{
    ...
    Console.WriteLine("    Provide text instructions (e.g. 'Select all squares with traffic lights').");
    Environment.Exit(1);
}

// Load image and base64-encode
Console.WriteLine($"[*] Loading grid image from {imageFile}...");
...
Console.WriteLine("[*] Submitting Grid Image CAPTCHA task...");
...
while (elapsed < maxTimeout)
{
    ...
    if (status == 1)
    {
        ...
        Console.WriteLine($"[+] Solved! Selected cells: {answer}");
        Console.WriteLine();
        Console.WriteLine("Next step: click the cells at the returned indices on");
        Console.WriteLine("the target page's grid and submit.");
        return;
    }
