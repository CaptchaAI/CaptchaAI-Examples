...
const CAPTCHA_IMAGE_FILE = process.env.CAPTCHA_IMAGE_FILE || "";
const CAPTCHA_INSTRUCTIONS = process.env.CAPTCHA_INSTRUCTIONS || "";
...
function validateConfig() {
  ...
  if (!CAPTCHA_IMAGE_FILE) {
    ...
    console.log("    Set the path to the grid CAPTCHA image file in .env.");
    process.exit(1);
  }
  ...
  if (!CAPTCHA_INSTRUCTIONS) {
    console.log("[!] ERROR: CAPTCHA_INSTRUCTIONS is not set.");
    console.log("    Provide text instructions (e.g. 'Select all squares with traffic lights').");
    process.exit(1);
  }
}
...
function loadImageBase64() {
  const imgPath = resolveImagePath();
  console.log(`[*] Loading grid image from ${CAPTCHA_IMAGE_FILE}...`);
  const data = fs.readFileSync(imgPath);
  return data.toString("base64");
}

async function submitTask(imageB64) {
  console.log("[*] Submitting Grid Image CAPTCHA task...");

  const params = new URLSearchParams();
  params.append("key", API_KEY);
  params.append("method", "post");
  params.append("body", imageB64);
  params.append("recaptcha", "1");
  params.append("textinstructions", CAPTCHA_INSTRUCTIONS);
  ...
  if (data.status !== 1) {
    ...
    if (AUTH_ERRORS.has(error)) {
      ...
      console.log("    Verify your image and instructions are correct.");
...
async function main() {
  ...
  const answer = await pollResult(taskId);
  console.log(`[+] Solved! Selected cells: ${answer}`);
  console.log();
  console.log("Next step: click the cells at the returned indices on");
  console.log("the target page's grid and submit.");
}
