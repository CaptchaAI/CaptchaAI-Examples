...
public class Solve {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (imageFile.isEmpty()) {
            ...
            System.out.println("    Set the path to the grid CAPTCHA image file in .env.");
            System.exit(1);
        }
        ...
        if (instructions.isEmpty()) {
            ...
            System.out.println("    Provide text instructions (e.g. 'Select all squares with traffic lights').");
            System.exit(1);
        }

        // Load image and base64-encode
        System.out.println("[*] Loading grid image from " + imageFile + "...");
        byte[] imageData = Files.readAllBytes(imagePath);
        String imageB64 = Base64.getEncoder().encodeToString(imageData);

        // Submit task via POST
        System.out.println("[*] Submitting Grid Image CAPTCHA task...");
        String postData = String.format(
            "key=%s&method=post&body=%s&recaptcha=1&textinstructions=%s&json=1",
        ...
        while (elapsed < maxTimeout) {
            ...
            if (status == 1) {
                ...
                System.out.println("[+] Solved! Selected cells: " + answer);
                System.out.println();
                System.out.println("Next step: click the cells at the returned indices on");
                System.out.println("the target page's grid and submit.");
                return;
            }
