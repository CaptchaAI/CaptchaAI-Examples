...
CAPTCHA_IMAGE_FILE = os.getenv("CAPTCHA_IMAGE_FILE", "")
CAPTCHA_INSTRUCTIONS = os.getenv("CAPTCHA_INSTRUCTIONS", "")
...
def validate_config() -> None:
    ...
    if not CAPTCHA_IMAGE_FILE:
        ...
        print("    Set the path to the grid CAPTCHA image file in .env.")
        sys.exit(1)
    ...
    if not os.path.isfile(image_path) and not os.path.isfile(CAPTCHA_IMAGE_FILE):
        ...
        sys.exit(1)
    if not CAPTCHA_INSTRUCTIONS:
        print("[!] ERROR: CAPTCHA_INSTRUCTIONS is not set.")
        print("    Provide text instructions (e.g. 'Select all squares with traffic lights').")
        sys.exit(1)
...
def load_image_base64() -> str:
    ...
    print(f"[*] Loading grid image from {CAPTCHA_IMAGE_FILE}...")
...
def submit_task(image_b64: str) -> str:
    ...
    print("[*] Submitting Grid Image CAPTCHA task...")
    payload = {
        "key": API_KEY,
        "method": "post",
        "body": image_b64,
        "recaptcha": "1",
        "textinstructions": CAPTCHA_INSTRUCTIONS,
        "json": "1",
    }
    ...
    if data.get("status") != 1:
        ...
        if error in AUTH_ERRORS:
            ...
            print("    Verify your image and instructions are correct.")
...
def main() -> None:
    ...
    print(f"[+] Solved! Selected cells: {answer}")
    print()
    print("Next step: click the cells at the returned indices on")
    print("the target page's grid and submit.")
