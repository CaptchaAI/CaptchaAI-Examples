...
func main() {
	...
	if imageFile == "" {
		...
		fmt.Println("    Set the path to the grid CAPTCHA image file in .env.")
		os.Exit(1)
	}
	...
	if instructions == "" {
		...
		fmt.Println("    Provide text instructions (e.g. 'Select all squares with traffic lights').")
		os.Exit(1)
	}

	// Load image and base64-encode
	fmt.Printf("[*] Loading grid image from %s...\n", imageFile)
	...
	fmt.Println("[*] Submitting Grid Image CAPTCHA task...")
	formData := url.Values{
		"key":              {apiKey},
		"method":           {"post"},
		"body":             {imageB64},
	...
	for elapsed < maxTimeout {
		...
		if result.Status == 1 {
			...
			fmt.Printf("[+] Solved! Selected cells: %s\n", answer)
			fmt.Println()
			fmt.Println("Next step: click the cells at the returned indices on")
			fmt.Println("the target page's grid and submit.")
			return
		}
