...
if image_file.empty?
  ...
  puts "    Set the path to the grid CAPTCHA image file in .env."
  exit 1
end
...
if instructions.empty?
  ...
  puts "    Provide text instructions (e.g. 'Select all squares with traffic lights')."
  exit 1
end
...
puts "[*] Loading grid image from #{image_file}..."
image_data = File.binread(image_path)
image_b64 = Base64.strict_encode64(image_data)

# Submit task via POST
puts "[*] Submitting Grid Image CAPTCHA task..."
uri = URI(SUBMIT_URL)
http = Net::HTTP.new(uri.host, uri.port)
http.use_ssl = true
http.open_timeout = 30
...
while elapsed < max_timeout
  ...
  if result_data["status"] == 1
    ...
    puts "[+] Solved! Selected cells: #{answer}"
    puts
    puts "Next step: click the cells at the returned indices on"
    puts "the target page's grid and submit."
    exit 0
  end
