...
fn main() {
    ...
    let image_file = get_env(&env, "CAPTCHA_IMAGE_FILE", "");
    let instructions = get_env(&env, "CAPTCHA_INSTRUCTIONS", "");
    ...
    println!("[*] Loading grid image from {}...", image_file);
    ...
    let post_body = format!(
        "key={}&method=post&body={}&recaptcha=1&textinstructions={}&json=1",
    ...
    while elapsed < max_timeout {
        ...
        if status == 1 {
            let answer = json_str(&result_resp, "request");
