# reCAPTCHA v3 Enterprise — CaptchaAI Example Pack

Full working example for solving reCAPTCHA v3 Enterprise using the CaptchaAI API.

reCAPTCHA v3 Enterprise combines Google's **Enterprise-grade** CAPTCHA with the **score-based** v3 model. It runs invisibly, returning a score between 0.0 (likely bot) and 1.0 (likely human). This example sends the required `enterprise`, `version`, `action`, and `min_score` parameters so the API returns a token that meets the target score.
...
**[How to Solve reCAPTCHA v3 Enterprise Using API](https://blog.captchaai.com/how-to-solve-recaptcha-v3-enterprise-using-api)**

## Prerequisites

- A CaptchaAI account with API key ([get one here](https://captchaai.com))
- A target page with reCAPTCHA v3 Enterprise for testing
...
1. Clone the repository:
   ...
   cd CaptchaAI-Examples/articles/how-to-solve-recaptcha-v3-enterprise-using-api
   ```
...
| `CAPTCHA_SITEKEY` | reCAPTCHA v3 Enterprise sitekey from the target page | Yes |
| `CAPTCHA_PAGEURL` | Full URL of the page with reCAPTCHA | Yes |
| `CAPTCHA_ACTION` | Action name used in `grecaptcha.enterprise.execute` (default: `verify`) | No |
| `CAPTCHA_MIN_SCORE` | Minimum acceptable score, 0.1–0.9 (default: `0.3`) | No |
| `CAPTCHA_DATA_S` | Optional `data-s` token from the page (leave empty if not present) | No |
| `POLL_INTERVAL` | Seconds between poll requests (default: 5) | No |
| `MAX_TIMEOUT` | Maximum seconds to wait for solution (default: 120) | No |

## How it works

1. **Submit** — Sends the CAPTCHA parameters to `https://ocr.captchaai.com/in.php` using method `userrecaptcha` with `enterprise=1`, `version=v3`, `action`, and `min_score`
...
```
[*] Submitting reCAPTCHA v3 Enterprise task...
...
- Make sure `.env` file is in the `articles/how-to-solve-recaptcha-v3-enterprise-using-api/` directory (not inside a language folder)
...
- [reCAPTCHA v3 Example](../how-to-solve-recaptcha-v3-automatically-using-api/) — Standard reCAPTCHA v3 solving
- [reCAPTCHA v2 Enterprise Example](../how-to-solve-recaptcha-v2-enterprise-using-api/) — Enterprise v2 solving
