...
captcha_action = get_env(env, "CAPTCHA_ACTION", "verify")
min_score = get_env(env, "CAPTCHA_MIN_SCORE", "0.3")
data_s = get_env(env, "CAPTCHA_DATA_S")
...
if sitekey.empty? || sitekey == "YOUR_SITE_KEY"
  ...
  puts "    Add the reCAPTCHA v3 Enterprise sitekey from your target page to .env"
  exit 1
end
...
# Submit task
puts "[*] Submitting reCAPTCHA v3 Enterprise task..."
submit_data = {
  key: api_key,
...
submit_data["data-s"] = data_s unless data_s.empty?
...
while elapsed < max_timeout
  ...
  if result_data["status"] == 1
    ...
    puts
    puts "Next step: inject this token into the target page's"
    puts "g-recaptcha-response hidden field and submit the form."
    exit 0
  end
