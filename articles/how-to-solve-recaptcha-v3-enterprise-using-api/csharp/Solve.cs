...
string captchaAction = GetEnv(env, "CAPTCHA_ACTION", "verify");
string minScore = GetEnv(env, "CAPTCHA_MIN_SCORE", "0.3");
string dataS = GetEnv(env, "CAPTCHA_DATA_S", "");
...
if (string.IsNullOrEmpty(sitekey) || sitekey == "YOUR_SITE_KEY")
{
    ...
    Console.WriteLine("    Add the reCAPTCHA v3 Enterprise sitekey from your target page to .env");
    Environment.Exit(1);
}
...
// Submit task
Console.WriteLine("[*] Submitting reCAPTCHA v3 Enterprise task...");
var submitQuery = $"key={Uri.EscapeDataString(apiKey)}&method=userrecaptcha&googlekey={Uri.EscapeDataString(sitekey)}&pageurl={Uri.EscapeDataString(pageurl)}&enterprise=1&version=v3&action={Uri.EscapeDataString(captchaAction)}&min_score={Uri.EscapeDataString(minScore)}&json=1";

if (!string.IsNullOrEmpty(dataS))
{
    submitQuery += $"&data-s={Uri.EscapeDataString(dataS)}";
}
