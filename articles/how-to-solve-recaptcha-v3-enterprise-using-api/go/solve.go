...
func main() {
	...
	captchaAction := getEnv(env, "CAPTCHA_ACTION", "verify")
	minScore := getEnv(env, "CAPTCHA_MIN_SCORE", "0.3")
	...
	if sitekey == "" || sitekey == "YOUR_SITE_KEY" {
		...
		fmt.Println("    Add the reCAPTCHA v3 Enterprise sitekey from your target page to .env")
		os.Exit(1)
	}
	...
	// Submit task
	fmt.Println("[*] Submitting reCAPTCHA v3 Enterprise task...")
	params := url.Values{
		"key":        {apiKey},
