...
var inputErrors = new HashSet<string> {
...
string imageFile = GetEnv(env, "CAPTCHA_IMAGE_FILE", "captcha.png");
...
Console.WriteLine("[*] Submitting Image CAPTCHA task...");
...
// Poll for result
Console.WriteLine("[*] Waiting 10s before first poll...");
...
while (elapsed < maxTimeout)
{
    ...
    if (status == 1)
    {
        string text = requestElement.ValueKind == JsonValueKind.String
            ? requestElement.GetString() ?? ""
            : requestElement.GetRawText();
        Console.WriteLine($"[+] Solved! Text: {text}");
        Console.WriteLine();
        Console.WriteLine("Next step: enter this text into the CAPTCHA input field");
        Console.WriteLine("on the target page and submit the form.");
        return;
    }
    ...
    if (solveErrors.Contains(request))
    {
        ...
        Console.WriteLine("    The CAPTCHA could not be solved. Verify the image and retry.");
        Environment.Exit(1);
    }
...
static void HandleError(string error, HashSet<string> auth, HashSet<string> balance, HashSet<string> input, HashSet<string> proxy)
{
    ...
    else if (input.Contains(error))
    {
        ...
        Console.WriteLine("    Verify your image file is valid and not too large.");
    }
