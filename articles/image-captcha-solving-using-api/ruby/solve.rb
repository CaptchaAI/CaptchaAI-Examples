...
image_file = get_env(env, "CAPTCHA_IMAGE_FILE", "captcha.png")
...
def handle_error(error)
  if AUTH_ERRORS.include?(error)
    ...
    puts "    Verify your image file is valid and not too large."
...
puts "[*] Submitting Image CAPTCHA task..."
uri = URI(SUBMIT_URL)
http = Net::HTTP.new(uri.host, uri.port)
http.use_ssl = true
http.open_timeout = 30
...
# Poll for result
puts "[*] Waiting 10s before first poll..."
sleep 10
...
while elapsed < max_timeout
  ...
  if result_data["status"] == 1
    text = result_data["request"]
    puts "[+] Solved! Text: #{text}"
    puts
    puts "Next step: enter this text into the CAPTCHA input field"
    puts "on the target page and submit the form."
    exit 0
  end
  ...
  if SOLVE_ERRORS.include?(error)
    ...
    puts "    The CAPTCHA could not be solved. Verify the image and retry."
    exit 1
  end
