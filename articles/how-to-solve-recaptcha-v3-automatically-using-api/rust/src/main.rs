...
fn main() {
    ...
    let captcha_action = get_env(&env, "CAPTCHA_ACTION", "verify");
    let min_score = get_env(&env, "CAPTCHA_MIN_SCORE", "0.3");
    ...
    if sitekey.is_empty() || sitekey == "YOUR_SITE_KEY" {
        ...
        println!("    Add the reCAPTCHA v3 sitekey from your target page to .env");
        std::process::exit(1);
    }
    ...
    let submit_url = format!(
        "{}?key={}&method=userrecaptcha&googlekey={}&pageurl={}&version=v3&action={}&min_score={}&json=1",
        SUBMIT_URL,
        urlencoding::encode(&api_key),
        urlencoding::encode(&sitekey),
        urlencoding::encode(&pageurl),
        urlencoding::encode(&captcha_action),
        urlencoding::encode(&min_score)
    );
