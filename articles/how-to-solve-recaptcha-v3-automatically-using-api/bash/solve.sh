...
SUBMIT_RESP=$(curl -sS --max-time 30 \
  "${SUBMIT_URL}?key=${ENCODED_KEY}&method=userrecaptcha&googlekey=${ENCODED_SITEKEY}&pageurl=${ENCODED_PAGEURL}&version=v3&action=${ENCODED_ACTION}&min_score=${ENCODED_MIN_SCORE}&json=1")
