...
if (string.IsNullOrEmpty(sitekey) || sitekey == "YOUR_SITE_KEY")
{
    ...
    Console.WriteLine("    Add the reCAPTCHA v3 sitekey from your target page to .env");
    Environment.Exit(1);
}
...
var submitQuery = $"key={Uri.EscapeDataString(apiKey)}&method=userrecaptcha&googlekey={Uri.EscapeDataString(sitekey)}&pageurl={Uri.EscapeDataString(pageurl)}&version=v3&action={Uri.EscapeDataString(captchaAction)}&min_score={Uri.EscapeDataString(minScore)}&json=1";
