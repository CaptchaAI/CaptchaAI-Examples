...
SUBMIT_RESPONSE=$(curl -s --max-time 30 \
    "${SUBMIT_URL}?key=${CAPTCHAAI_API_KEY}&method=userrecaptcha&googlekey=${CAPTCHA_SITEKEY}&pageurl=${CAPTCHA_PAGEURL}&enterprise=1&version=v3&action=verify&min_score=0.3&json=1")
