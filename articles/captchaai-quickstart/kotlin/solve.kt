...
fun main() {
    ...
    if (sitekey.isEmpty() || sitekey == "YOUR_SITE_KEY") {
        ...
        println("    Add the Turnstile sitekey from your target page to .env")
        exitProcess(1)
    }
    ...
    val submitParams = "key=${URLEncoder.encode(apiKey, enc)}" +
            "&method=turnstile" +
            "&sitekey=${URLEncoder.encode(sitekey, enc)}" +
            "&pageurl=${URLEncoder.encode(pageurl, enc)}" +
            "&json=1"

    val submitBody: String
    try {
        submitBody = httpGet("$SUBMIT_URL?$submitParams")
    } catch (e: Exception) {
        println("[!] Network error during submission: ${e.message}")
        exitProcess(1)
    }

    val submitStatus = parseStatus(submitBody)
    val submitRequest = parseRequest(submitBody)

    if (submitStatus != 1) {
        classifyError(submitRequest)
        exitProcess(1)
    }
    ...
    while (elapsed < maxTimeout) {
        ...
        val pollBody: String
        try {
            pollBody = httpGet("$RESULT_URL?$pollParams")
        } catch (e: Exception) {
        ...
        val rStatus = parseStatus(pollBody)
        val rRequest = parseRequest(pollBody)

        if (rStatus == 1) {
            val token = rRequest
            val truncated = if (token.length > 50) token.substring(0, 50) else token
            println("[+] Solved! Token: $truncated...")
            println("[+] Full token length: ${token.length} characters")
            println()
            println("Next step: inject this token into the target page's")
            println("cf-turnstile-response hidden field and submit the form.")
            exitProcess(0)
        }
        ...
        if (rRequest in SOLVE_ERRORS) {
            println("[!] Solve error: $rRequest")
            println("    The CAPTCHA could not be solved. Verify parameters and retry.")
            exitProcess(1)
        }

        if (rRequest in PROXY_ERRORS) {
            println("[!] Proxy error: $rRequest")
            println("    Check your proxy configuration or try a different proxy.")
            exitProcess(1)
        }

        println("[!] Unexpected error: $rRequest")
        exitProcess(1)
    }
