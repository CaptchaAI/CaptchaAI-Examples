...
class Solve
{
    ...
    public Solve(Dictionary<string, string> env)
    {
        ...
        _sitekey = GetEnv(env, "CAPTCHA_SITEKEY", "");
    ...
    private void ValidateConfig()
    {
        ...
        if (string.IsNullOrEmpty(_sitekey) || _sitekey == "YOUR_SITE_KEY")
        {
            Console.WriteLine("[!] ERROR: CAPTCHA_SITEKEY is not set.");
            Console.WriteLine("    Add the Turnstile sitekey from your target page to .env");
            Environment.Exit(1);
        }
        if (string.IsNullOrEmpty(_pageurl) || _pageurl == "https://example.com/login")
        {
    ...
    {
        Console.WriteLine("[*] Submitting Cloudflare Turnstile task...");
        var query = $"key={Uri.EscapeDataString(_apiKey)}" +
                    $"&method=turnstile" +
                    $"&sitekey={Uri.EscapeDataString(_sitekey)}" +
        ...
        try
        {
            body = await Client.GetStringAsync($"{SubmitUrl}?{query}");
        }
        catch (Exception ex)
        {
    ...
    private async Task<string> PollResultAsync(string taskId)
    {
        ...
        var query = $"key={Uri.EscapeDataString(_apiKey)}" +
                    $"&action=get&id={Uri.EscapeDataString(taskId)}&json=1";

        var elapsed = 15;
        var attempt = 0;
        var backoff = _pollInterval;
        ...
        while (elapsed < _maxTimeout)
        {
            ...
            try
            {
                body = await Client.GetStringAsync($"{ResultUrl}?{query}");
            }
            catch (Exception ex)
            {
    ...
    public static async Task Main(string[] args)
    {
        ...
        Console.WriteLine("cf-turnstile-response hidden field and submit the form.");
    }
}
