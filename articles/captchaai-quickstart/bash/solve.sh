...
if [[ -z "$CAPTCHA_SITEKEY" || "$CAPTCHA_SITEKEY" == "YOUR_SITE_KEY" ]]; then
    echo "[!] ERROR: CAPTCHA_SITEKEY is not set."
    echo "    Add the Turnstile sitekey from your target page to .env"
    exit 1
fi
if [[ -z "$CAPTCHA_PAGEURL" || "$CAPTCHA_PAGEURL" == "https://example.com/login" ]]; then
...
# Submit
echo "[*] Submitting Cloudflare Turnstile task..."
SUBMIT_RESPONSE=$(curl -s --max-time 30 \
    "${SUBMIT_URL}?key=${CAPTCHAAI_API_KEY}&method=turnstile&sitekey=${CAPTCHA_SITEKEY}&pageurl=${CAPTCHA_PAGEURL}&json=1")
