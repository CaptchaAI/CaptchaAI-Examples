...
if SITEKEY.empty? || SITEKEY == "YOUR_SITE_KEY"
  ...
  puts "    Add the Turnstile sitekey from your target page to .env"
  exit(1)
end
...
# Submit
puts "[*] Submitting Cloudflare Turnstile task..."
...
if data["status"] != 1
  error = data["request"] || "UNKNOWN_ERROR"
...
while elapsed < MAX_TIMEOUT
  ...
  poll_uri.query = URI.encode_www_form(
    key: API_KEY,
    action: "get",
    id: task_id,
    json: "1"
  ...
  if result["status"] == 1
    ...
    puts "cf-turnstile-response hidden field and submit the form."
    exit(0)
  end
