...
if [[ -z "$SITEKEY" || "$SITEKEY" == "YOUR_SITE_KEY" ]]; then
  ...
  echo "    Add the reCAPTCHA v2 Enterprise sitekey from your target page to .env"
  exit 1
fi
...
SUBMIT_QUERY="${SUBMIT_URL}?key=${ENCODED_KEY}&method=userrecaptcha&googlekey=${ENCODED_SITEKEY}&pageurl=${ENCODED_PAGEURL}&enterprise=1&json=1"

if [[ -n "$DATA_S" ]]; then
  ENCODED_DATA_S=$(python3 -c "import urllib.parse; print(urllib.parse.quote('$DATA_S'))" 2>/dev/null || echo "$DATA_S")
