...
public class Solve {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (sitekey.isEmpty() || sitekey.equals("YOUR_SITE_KEY")) {
            ...
            System.out.println("    Add the reCAPTCHA v2 Enterprise sitekey from your target page to .env");
            System.exit(1);
        }
        ...
        // Submit task
        System.out.println("[*] Submitting reCAPTCHA v2 Enterprise task...");
        StringBuilder submitParams = new StringBuilder();
        submitParams.append(String.format(
            "key=%s&method=userrecaptcha&googlekey=%s&pageurl=%s&enterprise=1&json=1",
            enc(apiKey), enc(sitekey), enc(pageurl)
        ));

        if (!dataS.isEmpty()) {
            submitParams.append("&data-s=").append(enc(dataS));
        }
