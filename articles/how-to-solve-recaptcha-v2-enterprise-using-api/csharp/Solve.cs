...
if (string.IsNullOrEmpty(sitekey) || sitekey == "YOUR_SITE_KEY")
{
    ...
    Console.WriteLine("    Add the reCAPTCHA v2 Enterprise sitekey from your target page to .env");
    Environment.Exit(1);
}
...
// Submit task
Console.WriteLine("[*] Submitting reCAPTCHA v2 Enterprise task...");
var submitQuery = $"key={Uri.EscapeDataString(apiKey)}&method=userrecaptcha&googlekey={Uri.EscapeDataString(sitekey)}&pageurl={Uri.EscapeDataString(pageurl)}&enterprise=1&json=1";
