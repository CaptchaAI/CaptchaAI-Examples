...
func handleError(error string) {
	if authErrors[error] {
		...
		fmt.Println("    Verify your gt (captcha_id) and page URL are correct.")
...
func main() {
	...
	if gt == "" || gt == "YOUR_CAPTCHA_ID" {
		fmt.Println("[!] ERROR: CAPTCHA_GT is not set.")
		fmt.Println("    Find the captcha_id in the page source or GeeTest v4 initialization call.")
		os.Exit(1)
	}
	...
	// Submit task
	fmt.Println("[*] Submitting GeeTest v4 task...")
	params := url.Values{
		"key":     {apiKey},
		"method":  {"geetest"},
		"gt":      {gt},
	...
	for elapsed < maxTimeout {
		...
		if result.Status == 1 {
			...
			fmt.Printf("[+] lot_number: %s\n", solution.LotNumber)
			fmt.Printf("[+] pass_token: %s\n", solution.PassToken)
			fmt.Printf("[+] gen_time: %s\n", solution.GenTime)
			fmt.Printf("[+] captcha_output: %s\n", solution.CaptchaOutput)
			fmt.Println()
			fmt.Println("Next step: submit these five values to the site's")
			fmt.Println("GeeTest v4 verification endpoint.")
			return
		}
