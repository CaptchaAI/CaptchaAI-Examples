...
fn main() {
    ...
    if gt.is_empty() || gt == "YOUR_CAPTCHA_ID" {
    ...
    let submit_url = format!(
        "{}?key={}&method=geetest&gt={}&pageurl={}&version=4&json=1",
        SUBMIT_URL,
    ...
    while elapsed < max_timeout {
        ...
        if status == 1 {
            ...
            let captcha_id = json_str(&solution_json, "captcha_id");
            let lot_number = json_str(&solution_json, "lot_number");
            let pass_token = json_str(&solution_json, "pass_token");
            let gen_time = json_str(&solution_json, "gen_time");
            let captcha_output = json_str(&solution_json, "captcha_output");
            println!("[+] Solved!");
            println!("[+] captcha_id: {}", captcha_id);
            println!("[+] lot_number: {}", lot_number);
            println!("[+] pass_token: {}", pass_token);
            println!("[+] gen_time: {}", gen_time);
            println!("[+] captcha_output: {}", captcha_output);
            println!();
