...
fun handleError(error: String) {
    when {
        ...
        error in INPUT_ERRORS -> {
            ...
            println("    Verify your gt (captcha_id) and page URL are correct.")
        }
...
fun main() {
    ...
    if (gt.isEmpty() || gt == "YOUR_CAPTCHA_ID") {
        println("[!] ERROR: CAPTCHA_GT is not set.")
        println("    Find the captcha_id in the page source or GeeTest v4 initialization call.")
        System.exit(1)
    }
    ...
    // Submit task
    println("[*] Submitting GeeTest v4 task...")
    val submitParams = "key=${enc(apiKey)}&method=geetest&gt=${enc(gt)}&pageurl=${enc(pageurl)}&version=4&json=1"
    ...
    while (elapsed < maxTimeout) {
        ...
        if (status == 1) {
            ...
            val captchaId = jsonStr(solutionJson, "captcha_id")
            val lotNumber = jsonStr(solutionJson, "lot_number")
            val passToken = jsonStr(solutionJson, "pass_token")
            val genTime = jsonStr(solutionJson, "gen_time")
            val captchaOutput = jsonStr(solutionJson, "captcha_output")
            println("[+] Solved!")
            println("[+] captcha_id: $captchaId")
            println("[+] lot_number: $lotNumber")
            println("[+] pass_token: $passToken")
            println("[+] gen_time: $genTime")
            println("[+] captcha_output: $captchaOutput")
            println()
            println("Next step: submit these five values to the site's")
            println("GeeTest v4 verification endpoint.")
            return
        }
