...
if gt.empty? || gt == "YOUR_CAPTCHA_ID"
  puts "[!] ERROR: CAPTCHA_GT is not set."
  puts "    Find the captcha_id in the page source or GeeTest v4 initialization call."
  exit 1
end
...
def handle_error(error)
  if AUTH_ERRORS.include?(error)
    ...
    puts "    Verify your gt (captcha_id) and page URL are correct."
...
# Submit task
puts "[*] Submitting GeeTest v4 task..."
submit_params = {
  key: api_key,
  method: "geetest",
  gt: gt,
...
while elapsed < max_timeout
  ...
  if result_data["status"] == 1
    ...
    puts "[+] Solved!"
    puts "[+] captcha_id: #{solution['captcha_id']}"
    puts "[+] lot_number: #{solution['lot_number']}"
    puts "[+] pass_token: #{solution['pass_token']}"
    puts "[+] gen_time: #{solution['gen_time']}"
    puts "[+] captcha_output: #{solution['captcha_output']}"
    puts
    puts "Next step: submit these five values to the site's"
    puts "GeeTest v4 verification endpoint."
    exit 0
  end
